...
  
  const isTransparentPage = location.pathname === '/about' || location.pathname === '/contact' || location.pathname === '/properties';
...
//     { name: 'Text', to: '/text' },
    // { name: 'Emi-Calculator', to: '/emiCalculator' },
    
  ];
...
    if (isTransparent) {
      return `${baseClasses} ${isActive ? 'text-white underline' : 'text-gray-300'}`;
    }
...
          {/* LOGO SIZE FIX: h-10 for mobile, lg:h-18 for laptop/desktop */}
          <img
            src={logo}
            alt="The One Property Logo"
            className="h-10 lg:h-18 w-auto object-contain flex-shrink-0"
          />
          {/* LOGO TEXT SIZE FIX: text-xs for mobile for better fit */}
          <span className={`text-xs font-light ${isTransparent ? 'text-gray-300' : 'text-gray-600'}`}>
            The One Property
          </span>
        </Link>
...
            className={`${isTransparent ? 'text-white' : 'text-gray-700'} focus:outline-none`}
          >
            <svg
              className="h-6 w-6"
              fill="none"
